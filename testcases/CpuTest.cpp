...
void setRegister(Memory& memory, Cpu& cpu, int registerNumber, uint8_t registerValue) {
    uint16_t setRegisterOpcode =
        (uint16_t)((6 << OpcodeBitshifts::NIBBLE_THREE) | (registerNumber << OpcodeBitshifts::NIBBLE_TWO) | registerValue);
...
void expectInstructionSkipped(Cpu& cpu) {
    ...
    // each instruction moves the program counter by DEFAULT_NUM_INSTRUCTIONS_PER_CYCLE. The skip instruction opcode moves the program
    ...
    EXPECT_EQ(newProgramCounter,
              previousProgramCounter + Cpu::DEFAULT_NUM_INSTRUCTIONS_PER_CYCLE + Cpu::DEFAULT_NUM_INSTRUCTIONS_PER_CYCLE);
}
...
TEST_F(CpuTestFixture, SkipEquals) {
    ...
    uint16_t skipEqualsOpcode =
        (uint16_t)((3 << OpcodeBitshifts::NIBBLE_THREE) | (registerNumber << OpcodeBitshifts::NIBBLE_TWO) | equalityValue);
    ...
    uint16_t skipEqualsFailOpcode =
        (uint16_t)((3 << OpcodeBitshifts::NIBBLE_THREE) | (registerNumber << OpcodeBitshifts::NIBBLE_TWO) | inequalityValue);
...
TEST_F(CpuTestFixture, SkipNotEquals) {
    ...
    uint8_t value1 = 0x10;
    uint8_t value2 = 0x11;
    uint16_t skipNotEqualsOpcode =
        (uint16_t)((4 << OpcodeBitshifts::NIBBLE_THREE) | (registerNumber << OpcodeBitshifts::NIBBLE_TWO) | value1);
    ...
    uint16_t skipNotEqualsFailOpcode =
        (uint16_t)((4 << OpcodeBitshifts::NIBBLE_THREE) | (registerNumber << OpcodeBitshifts::NIBBLE_TWO) | value2);
...
TEST_F(CpuTestFixture, SkipRegisterEquals) {
    ...
    int register2 = 1;
    uint8_t equalityValue = 0x10;
    uint16_t skipRegisterEqualsOpcode = (uint16_t)((5 << OpcodeBitshifts::NIBBLE_THREE) | (register1 << OpcodeBitshifts::NIBBLE_TWO) |
...
TEST_F(CpuTestFixture, AddRegister) {
    ...
    uint16_t addRegisterOpcode =
        (uint16_t)((7 << OpcodeBitshifts::NIBBLE_THREE) | (registerNumber << OpcodeBitshifts::NIBBLE_TWO) | addedValue);
...
void testArithmeticOperator(Memory& memory, Cpu& cpu, uint8_t valueX, uint8_t valueY, uint8_t expectedOutput,
    ...
    uint16_t setRegisterOrOpcode = (uint16_t)((8 << OpcodeBitshifts::NIBBLE_THREE) | (registerNumberX << OpcodeBitshifts::NIBBLE_TWO) |
...
TEST_F(CpuTestFixture, skipRegistersNotEqual) {
    ...
    uint16_t skipRegistersNotEqualOpcode =
        (uint16_t)((9 << OpcodeBitshifts::NIBBLE_THREE) | (registerNumberX << OpcodeBitshifts::NIBBLE_TWO) |
...
TEST_F(CpuTestFixture, jumpToAddressPlusRegisterZero) {
    ...
    // TODO: see what's supposed to happen if the opcode goes over the allowed address range. Is the program counter supposed to overflow
    // and reset?
}
...
TEST_F(CpuTestFixture, DrawSprite) {
    ...
    uint16_t drawSpriteOpcode = (uint16_t)((0xD << OpcodeBitshifts::NIBBLE_THREE) | (spriteCoordinateX << OpcodeBitshifts::NIBBLE_TWO) |
                                           (spriteCoordinateY << OpcodeBitshifts::NIBBLE) | spriteHeight);
...
TEST_F(CpuTestFixture, skipOnKeyPressed) {
    ...
    uint16_t skipOnKeyPressedOpcode =
        (uint16_t)((0xE << OpcodeBitshifts::NIBBLE_THREE) | (registerNumberX << OpcodeBitshifts::NIBBLE_TWO) | 0x9E);
    ...
    EXPECT_CALL(inputController, isKeyPressed(buttonNumber)).WillOnce(Return(true)).WillOnce(Return(false));
...
TEST_F(CpuTestFixture, skipOnKeyNotPressed) {
    unsigned int registerNumberX = 0;
    uint8_t buttonNumber = 0;
    uint16_t skipOnKeyNotPressedOpcode =
        (uint16_t)((0xE << OpcodeBitshifts::NIBBLE_THREE) | (registerNumberX << OpcodeBitshifts::NIBBLE_TWO) | (0xA1));
    setRegister(memory, cpu, registerNumberX, buttonNumber);

    EXPECT_CALL(inputController, isKeyPressed(buttonNumber)).WillOnce(Return(false)).WillOnce(Return(true));
...
TEST_F(CpuTestFixture, setRegisterToDelayTimer) {
    ...
    uint16_t setRegisterToDelayTimerOpcode =
        (uint16_t)((0xF << OpcodeBitshifts::NIBBLE_THREE) | (registerNumberX << OpcodeBitshifts::NIBBLE_TWO) | 0x07);
...
TEST_F(CpuTestFixture, waitForKeyPress) {
    ...
    EXPECT_CALL(inputController, waitForKeyPress()).WillOnce(Return(keyPressed));
    uint16_t waitForKeyPressOpcode =
        (uint16_t)((0xF << OpcodeBitshifts::NIBBLE_THREE) | (registerNumberX << OpcodeBitshifts::NIBBLE_TWO) | 0x0A);
...
TEST_F(CpuTestFixture, setDelayTimer) {
    ...
    uint16_t setDelayTimerOpcode =
        (uint16_t)((0xF << OpcodeBitshifts::NIBBLE_THREE) | (registerNumberX << OpcodeBitshifts::NIBBLE_TWO) | 0x15);
...
TEST_F(CpuTestFixture, setSoundTimer) {
    unsigned int registerNumberX = 0;
    uint8_t sound = 0x90;
    uint16_t setSoundTimerOpcode =
        (uint16_t)((0xF << OpcodeBitshifts::NIBBLE_THREE) | (registerNumberX << OpcodeBitshifts::NIBBLE_TWO) | 0x18);
...
TEST_F(CpuTestFixture, incrementIndexRegister) {
    ...
    uint16_t incrementIndexRegisterOpcode =
        (uint16_t)((0xF << OpcodeBitshifts::NIBBLE_THREE) | (registerNumberX << OpcodeBitshifts::NIBBLE_TWO) | 0x1E);
...
TEST_F(CpuTestFixture, setIndexToSpriteLocation) {
    ...
    for (uint8_t i = 0; i < maxEightBitIntValue; i++) {
        uint16_t setIndexToSpriteLocationOpcode =
            (uint16_t)((0xF << OpcodeBitshifts::NIBBLE_THREE) | (registerNumberX << OpcodeBitshifts::NIBBLE_TWO) | 0x29);
...
TEST_F(CpuTestFixture, convertToBcd) {
    ...
    uint16_t convertToBcdOpcode =
        (uint16_t)((0xF << OpcodeBitshifts::NIBBLE_THREE) | (registerNumberX << OpcodeBitshifts::NIBBLE_TWO) | (0x33));
...
void testRegisterDump(Memory& memory, Cpu& cpu, unsigned int numRegisters) {
    ...
    uint16_t registerDumpOpcode =
        (uint16_t)((0xF << OpcodeBitshifts::NIBBLE_THREE) | (registerNumberX << OpcodeBitshifts::NIBBLE_TWO) | 0x55);
...
void testRegisterLoad(Memory& memory, Cpu& cpu, unsigned int numRegisters) {
    ...
    int registerNumberX = numRegisters - 1;
    uint16_t registerLoadOpcode =
        (uint16_t)((0xF << OpcodeBitshifts::NIBBLE_THREE) | (registerNumberX << OpcodeBitshifts::NIBBLE_TWO) | 0x65);
